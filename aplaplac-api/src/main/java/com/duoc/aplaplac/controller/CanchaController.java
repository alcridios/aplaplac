...
public class CanchaController {
	...
	@PostMapping(value = "")
	public ResponseEntity<Long> saveCancha(@RequestBody CanchaDTO cancha) {
		CanchaDTO savedCancha = canchaService.save(cancha);
		return new ResponseEntity<>(savedCancha.getId(), HttpStatus.CREATED);
	}

	@PutMapping(value = "/{id}")
	public ResponseEntity<Long> updateCancha(@PathVariable Long id, @RequestBody CanchaDTO cancha) {
		cancha.setId(id);
